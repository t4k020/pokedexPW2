...
// Evitamos inyección básica escapando el término
$termino_safe = $conn->real_escape_string($termino);

// Consulta para buscar por nombre o número
...
WHERE P.nombre LIKE '%$termino_safe%' OR P.idNoIncremental = '$termino_safe'
GROUP BY P.idNoIncremental
ORDER BY P.idNoIncremental ASC";

$result = $conn->query($sql);

// Si no encuentra nada, activa el flag de error y trae todos
...
<div class="container my-5">
        ...
        <div class="col-12 col-md-6">
            <form action="buscar.php" method="GET">
                <div class="input-group shadow">
                    <input type="search" name="termino" class="form-control bg-dark text-white border-secondary ps-3"
                           placeholder="Buscar Pokémon..." value="<?php echo htmlspecialchars($termino); ?>">
    ...
    <?php if ($error_busqueda): ?>
        <div class="row justify-content-center mb-4">
            <div class="col-12 col-md-8 text-center">
                <div class="alert alert-danger shadow border-0 py-3" role="alert">
                    <i class="bi bi-exclaim-circle-fill fs-5 me-2"></i>
                    <span class="fw-bold">Pokémon no encontrado.</span>
                    <div class="text-muted small mt-1">Mostrando todos los Pokémon disponibles a continuación:</div>
                </div>
            </div>
        </div>
    <?php endif; ?>
    ...
    <?php if (isset($_SESSION['nombre']) && $_SESSION['nombre'] === 'admin'): ?>
        <a class="btn btn-success mb-4 fw-bold shadow-sm" href="crear_formulario.php">
            + CREAR POKEMON
        </a>
    <?php endif; ?>
